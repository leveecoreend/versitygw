...
echo_versions() {
  ...
  if ! keys=$(xmllint --xpath "//*[local-name()=\"$1\"]/*[local-name()=\"$2\"]/text()" "$3" 2>&1); then
    if [[ "$keys" == *"XPath set is empty"* ]]; then
      return 0
    fi
    log 2 "error getting Version '$1', '$2' values: $keys"
    return 1
  fi
...
parse_versions_rest() {
  ...
  version_ids=()
  version_islatests=()
  if ! keys=$(echo_versions "Version" "Key" "$1" 2>&1); then
    log 2 "error getting Version Key values: $keys"
    return 1
  fi
  # shellcheck disable=SC2206
  version_keys+=($keys)
  if ! ids=$(echo_versions "Version" "VersionId" "$1" 2>&1); then
    log 2 "error getting Version VersionId values: $ids"
    return 1
  fi
  # shellcheck disable=SC2206
  version_ids+=($ids)
  if ! is_latest=$(echo_versions "Version" "IsLatest" "$1" 2>&1); then
    log 2 "error getting Version IsLatest values: $is_latest"
    return 1
  fi
  # shellcheck disable=SC2206
  version_islatests+=($is_latest)
  if ! keys=$(echo_versions "DeleteMarker" "Key" "$1" 2>&1); then
    log 2 "error getting DeleteMarker Key values: $keys"
    return 1
  fi
  # shellcheck disable=SC2206
  version_keys+=($keys)
  if ! ids=$(echo_versions "DeleteMarker" "VersionId" "$1" 2>&1); then
    log 2 "error getting DeleteMarker VersionId values: $ids"
    return 1
  fi
  # shellcheck disable=SC2206
  version_ids+=($ids)
  if ! is_latest=$(echo_versions "DeleteMarker" "IsLatest" "$1" 2>&1); then
