...
create_large_file() {
  ...
  file_size=${2:-160}
  if ! create_large_file_with_size "$1" "$file_size"; then
...
create_test_file_base() {
  ...
  if [[ "$requested_size" -eq 0 ]]; then
    ...
    dd if=/dev/urandom of="$full_path" bs="$requested_size" count=1 conv=notrunc 2>&1
  else
    ...
    dd if=/dev/urandom of="$full_path" bs="$block_size" count="$count" conv=notrunc 2>&1

    # If there's a remainder (e.g., 1.5MB), append the last few bytes
    if [[ $remainder -gt 0 ]]; then
      dd if=/dev/urandom of="$full_path" bs=1 count="$remainder" oflag=append conv=notrunc 2>&1
    fi
  fi
  echo "$file_name"
  return 0
...
create_test_file_count() {
  ...
  if ! file_prefix=$(get_file_name 2>&1); then
    log 2 "error getting file prefix: $file_prefix"
    return 1
  fi
  for ((i=1;i<=$1;i++)) {
