...
check_rest_go_expected_error() {
  ...
  result="$(cat "$1")"
  if ! status_line=$(bypass_continues "$result" 2>&1); then
    log 2 "error bypassing continues: $status_code"
    return 1
  fi
  status_code=$(echo -n "$status_line" | awk '{print $2}')
...
check_rest_expected_header_error() {
  ...
  result="$(cat "$1")"
  if ! status_line=$(bypass_continues "$result" 2>&1); then
    log 2 "error bypassing continues: $status_line"
    return 1
  fi
