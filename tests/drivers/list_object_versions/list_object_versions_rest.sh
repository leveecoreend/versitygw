...
parse_a_non_latest_version_id() {
  ...
  if ! version_id=$(parse_version_id "$1" "false" 2>&1); then
    log 2 "error getting non-latest version ID: $version_id"
    return 1
  fi
  echo "$version_id"
  return 0
...
get_a_non_latest_version() {
  ...
  if ! non_latest_version=$(send_rest_go_command_callback "200" "parse_a_non_latest_version_id" "-method" "GET" "-query" "versions=" "-bucketName" "$1" 2>&1); then
    log 2 "error retrieving non-latest version ID: $non_latest_version"
    return 1
  fi
...
parse_latest_version_id() {
  ...
  if ! version_id=$(parse_version_id "$1" "true" 2>&1); then
    log 2 "error getting latest version ID: $version_id"
    return 1
  fi
  log 5 "version ID: $version_id"
  echo "$version_id"
